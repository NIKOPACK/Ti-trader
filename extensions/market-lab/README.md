...
- `generate_trade_signal`：默认用 `ema-cross` 返回非绑定的 bullish/bearish/neutral/insufficient-data 偏向。它是分析，不是交易授权。
- `evaluate_strategy`：对命名预设评分：`ema-cross`、`rsi-revert`、`macd-hist`。只返回 `bias`、`event`、`reasons` 和失效参考，不下单。
