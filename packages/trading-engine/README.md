...
- protection predicates and coverage helpers for stop-loss and related orders (`isProtection`, `protectionCoverage`, `isProtectiveExit`);
- optional account-wide hard-risk admission (`risk.account`): exposure, leverage, margin, freshness, execution price/liquidity, protection coverage, and flow-adjusted loss/drawdown with latched hard-loss trips;
- `TradingEngine` for planning, risk checks, confirmation policy, submission, reservation settlement, and independent risk supervision.
...
Paper mode is the safe default in Ti. Paper Futures only applies a simplified maintenance-margin liquidation boundary; it does not model exchange-specific liquidation behavior, funding, slippage, or partial fills, so its results must not be interpreted as a realistic leveraged-trading simulation. Live execution requires exchange credentials and an explicit submission policy: callers must provide `confirm` or deliberately opt in with `allowUnconfirmedLive` for a headless workflow. A confirmation is bound to the latest preflight evidence snapshot; material changes to price, risk notional or capability warnings produce a new summary and require another confirmation instead of submitting the originally confirmed numbers. Invalidating changes, TTL expiry and unattended drift beyond 1% still fail closed. Risk limits are enforced by the engine rather than treated as an LLM prompt convention. Confirmation cancellation and definite submission rejection release reservations atomically with the journal result. Unknown submission or failed post-submission persistence retains unresolved execution evidence and its claim; do not retry the original order.

Entry admission checks run before preflight, inside risk reservation, and after confirmation immediately before the exchange submission starts. Storage-read failures also block new entries. Validated spot sells, sell OCOs and futures reductions remain available under their existing limits and confirmation policy, as do cancellations.

Live cancellation without `risk.account` refuses to remove a stop-flavored exit that still protects an open position; the call fails closed and directs the operator to a controlled close. When account hard risk is configured, that supervisor arbitrates cancellations, including required protection. `isProtectiveExit` is part of the public surface; `verifiedReducingOrder` accepts a position-mode-only scope.
...
Unreleased execution references bind an immutable application plan version and stable intent before submission. Referenced records remain protected from ordinary terminal pruning until the consumer durably archives and acknowledges the exact revision; linked open orders remain refreshable. `refreshExecutionEvidence()` performs correlated reads only. Actual order or fee changes produce a new revision, but identical polling does not change the revision, observation timestamp, or settled quota.

`Order.feeObservation` and execution evidence orders preserve `{ source, completeness, charges }`. Sources are `exchange` or `paper-ledger`; charges retain original currency and signed cost, including explicit zero and rebates. Completeness covers the currently filled quantity, not future fills. Paper persists the charged ledger amount; legacy recovery requires an exact unique trade/order match. Supported live order-trade lookups are bounded and remain incomplete when history cannot be reconciled.

Use `observedExecutionFee(record)` for quote-denominated totals. It requires non-operator evidence, terminal uniquely identified orders, complete mode-matching observations and quote-only charges. Missing or partial costs, base/third-token fees and legacy numeric `fee` alone return `undefined`, never an assumed zero. Funding, slippage and currency conversion are not inferred.

`Ticker.sourceTimestampKnown` is true only for a valid venue-provided timestamp. A local fallback can still support the existing display contract, but it cannot establish source freshness for prospective studies. Paper uses the same public-market timestamp provenance.
