...
- Trailing stop: trailing_stop_market with trailingPercent trails the best price since placement (sell: peak; buy: trough) and fires after the given percent pullback. ${config.mode === "paper" ? "In paper mode triggers are evaluated on every account read and by the background monitor, with gaps backfilled from klines." : "Trailing support depends on the exchange; the order is rejected if unsupported."}
- place_oco: one-cancels-the-other bracket — a stop-loss AND a take-profit for the same amount with a single reservation; when one leg fills the other is cancelled. Use it to protect a filled entry in both directions. ${config.mode === "live" ? "Live OCO support depends on the exchange (okx native); if rejected, fall back to a single stop-loss." : ""}
