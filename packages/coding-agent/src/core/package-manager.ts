...
export class DefaultPackageManager implements PackageManager {
	...
	async resolve(onMissing?: (source: string) => Promise<MissingSourceAction>): Promise<ResolvedPaths> {
		...
		const projectBaseDir = join(this.cwd, this.settingsManager.getProjectConfigDirName());
	...
	private getNpmInstallRoot(scope: SourceScope, temporary: boolean): string {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.settingsManager.getProjectConfigDirName(), "npm");
		}
	...
	private getManagedNpmInstallPath(source: NpmSource, scope: SourceScope): string {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.settingsManager.getProjectConfigDirName(), "npm", "node_modules", source.name);
		}
	...
	private getGitInstallRoot(scope: SourceScope): string | undefined {
		...
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.settingsManager.getProjectConfigDirName(), "git");
		}
	...
	private getBaseDirForScope(scope: SourceScope): string {
		if (scope === "project") {
			this.assertProjectTrustedForScope(scope);
			return join(this.cwd, this.settingsManager.getProjectConfigDirName());
		}
		if (scope === "user") {
