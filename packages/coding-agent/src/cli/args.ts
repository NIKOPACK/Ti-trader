...
${chalk.bold("Commands:")}
  ...
  ${APP_NAME} auth <command>            Export credentials or check provider readiness
...
${chalk.bold("Examples:")}
  ...
  ${APP_NAME} auth print-api-key --provider openai --output-file /tmp/openai-key

  # Print an OAuth bearer token for an external client (refreshes if expired)
  ${APP_NAME} auth print-bearer-token --provider openai-codex --output-file /tmp/openai-token
