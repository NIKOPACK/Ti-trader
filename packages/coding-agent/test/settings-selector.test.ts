...
describe("SettingsSelectorComponent", () => {
	...
	it("localizes the selected description without changing the setting label", () => {
		const config = {
			autoCompact: true,
			defaultModel: "not set",
			availableDefaultModels: [],
		...
		const localizer = (key: string, fallback: string) =>
			key === "settings.auto-compact" ? "上下文过大时自动压缩" : fallback;
		const callbacks = {} as unknown as SettingsCallbacks;

		const lines = new SettingsSelectorComponent(config, callbacks, localizer)
			.getSettingsList()
			.render(120)
			.join("\n");
		expect(lines).toContain("Auto-compact");
		expect(lines).toContain("上下文过大时自动压缩");
	});
});
